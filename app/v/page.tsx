...
/* Tipos e componentes mínimos para render */
...
function Button({ label, cfg }: { label:string; cfg:ThemeConfig }) {
  ...
  const cn = "w-full font-semibold px-4 py-3 border transition-transform active:translate-y-px " + (cfg.button.variant === "ghost" ? "hover:bg-white/10" : "hover:opacity-90");
            ...
            <div key={cat.id}>
              ...
              <div className="space-y-3">
                {(cat.limit ? cat.items.slice(0, cat.limit) : cat.items).map(l => (
                  <a key={l.id} href={l.url} target="_blank" className="block"><Button label={l.label} cfg={cfg} /></a>
                ))}
              </div>
            </div>
          ))}
