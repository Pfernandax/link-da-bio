...
/** Tipos mínimos (iguais aos do editor) */
type SocialIcon =
...
/** Decodifica o que foi gerado no editor (btoa + encodeURIComponent) */
function decodeParam(encoded: string): PageConfig | null {
  try {
...
const Icon = ({ name, className = "w-5 h-5" }: { name: SocialIcon; className?: string }) => {
  ...
  switch (name) {
    case "instagram": return <svg {...common} viewBox="0 0 24 24"><path d="M7 2h10a5 5 0 015 5v10a5 5 0 01-5 5H7a5 5 0 01-5-5V7a5 5 0 015-5zm10.5 3.75a1.25 1.25 0 100 2.5 1.25 1.25 0 000-2.5zM12 7a5 5 0 100 10 5 5 0 000-10z"/></svg>;
    case "whatsapp": return <svg {...common} viewBox="0 0 24 24"><path d="M20.5 3.5A11 11 0 103.48 20.52L2 22l1.6-.47A11 11 0 1020.5 3.5z"/></svg>;
    case "youtube":  return <svg {...common} viewBox="0 0 24 24"><path d="M23.5 6.5s-.23-1.64-.95-2.36c-.91-.95-1.93-.95-2.4-1C16.97 3 12 3 12 3S7.03 3 3.85 3.14c-.47.05-1.49.05-2.4 1C.73 4.86.5 6.5.5 6.5S.36 8.42.36 10.34v1.32C.36 13.58.5 15.5.5 15.5s.23 1.64.95 2.36c.91.95 2.1.92 2.64 1.02C6.56 19.06 12 19.1 12 19.1s4.97-.01 8.15-.15c.47-.05 1.49-.05 2.4-1 .72-.72.95-2.36.95-2.36s.14-1.92.14-3.84V10.34c0-1.92-.14-3.84-.14-3.84zM9.75 14.3V8.7L15 11.5l-5.25 2.8z"/></svg>;
    case "tiktok":   return <svg {...common} viewBox="0 0 24 24"><path d="M14 3c.7 2.3 2.3 4 4.5 4.5V11a7 7 0 11-7-7h2.5z"/></svg>;
    case "telegram": return <svg {...common} viewBox="0 0 24 24"><path d="M9.04 16.46l-.39 5.49 2.71-2.6 5.43 3.97c.99.55 1.69.26 1.96-.92l3.55-16.63c.31-1.46-.53-2.03-1.49-1.68L1.7 9.66c-1.45.56-1.43 1.37-.25 1.74l5.45 1.7L19.66 6.2c.6-.41 1.15-.18.7.23"/></svg>;
    case "x":        return <svg {...common} viewBox="0 0 24 24"><path d="M4 4l16 16M20 4L4 20"/></svg>;
    case "facebook": return <svg {...common} viewBox="0 0 24 24"><path d="M13 22V12h3l1-4h-4V6a2 2 0 012-2h2V1h-3a5 5 0 00-5 5v2H7v4h3v10h3z"/></svg>;
    case "site":     return <svg {...common} viewBox="0 0 24 24"><path d="M3 4h18v16H3zM3 9h18"/></svg>;
...
export default function PublicPage() {
  ...
  const page = decodeParam(String(data));
    ...
    return (
      <div className="min-h-screen grid place-items-center bg-[#0b1020] text-white">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 max-w-md text-center">
          ...
          <p className="opacity-80 text-sm">O conteúdo não pôde ser carregado. Gere o link novamente no editor.</p>
        </div>
      </div>
    );
  }
        ...
        <div className="text-center mb-5">
          ...
          {page.socials.length > 0 && (
            <div className="flex items-center justify-center gap-3 mt-3">
              {page.socials.map(s => (
                <a key={s.id} href={s.url} target="_blank" className="p-2 rounded-xl" style={{ background: "rgba(255,255,255,.08)", border: "1px solid rgba(255,255,255,.12)" }}>
                  <Icon name={s.type} />
                </a>
              ))}
            </div>
